...
export default function Toolbar({ isActive, fullScreen, centering, pause }) {
    ...
    document.addEventListener('fullscreenchange', () => {
        if (document.fullscreenElement == null) {
            Toolbar.current.style.transform = 'translateY(0%)'
        }
    }
    )
    ...
    useEffect(() => {
        ...
        !isActive ? Toolbar.current.style.transform = 'translateY(-100%)' : Toolbar.current.style.transform = 'translateY(0%)'
    }, [isActive])
    ...
    return (
        <div className="Toolbar" ref={Toolbar}>
            ...
            <button className="Toolbar__btn" onClick={pause}>
                {
                    isPlaying ? <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none">
                        <rect x="8" y="6" width="3" height="12" rx="0.5" stroke="#fff" />
                        <rect x="13" y="6" width="3" height="12" rx="0.5" stroke="#fff" />
                    </svg> : <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none">
                        <path d="M8 5V19L19 12L8 5Z" stroke="#fff" strokeWidth="1.5" fill="none" strokeLinejoin="round" />
                    </svg>
                }
            </button>
        </div>
